...
AICORE void runTLog(__gm__ T __out__ *out, __gm__ T __in__ *src0)
{
...
void LaunchTLog(T *out, T *src0, void *stream)
{
    if constexpr (std::is_same_v<T, aclFloat16>)
        runTLog<half, kGRows_, kGCols_, kTRows_, kTCols_>((half *)(out), (half *)(src0));
    else
        runTLog<T, kGRows_, kGCols_, kTRows_, kTCols_>(out, src0);
}
const int NUM_16 = 16;
const int NUM_64 = 64;
const int NUM_256 = 256;
template void LaunchTLog<float, NUM_64, NUM_64, NUM_64, NUM_64>(float *out, float *src0, void *stream);
